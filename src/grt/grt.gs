...
(define (constant-writer-add! cw cnst)
  (let ((bv (new-bytevector 0)))
    (cond
      ((or (nil? cnst)
           (boolean? cnst))
       (bytevector-push! bv const-direct)
       (write-u64le!
        bv
        (case cnst
          ((()) 7) ;; nil
...
(define (code-body-writer->code cw)
  (bytevector->bytestring (car cw)))
(define (cbw-ensure-locals! cw locals)
  (box-swap! (caddr cw) max locals))
